...
class CassetteWriter(EventHandler):
    ...
    def __post_init__(self) -> None:
        ...
        self.worker = threading.Thread(
            name="SchemathesisCassetteWriter", target=writer, kwargs=kwargs
        )
        self.worker.start()
...
http_interactions:"""
            ...
            for interaction in item.interactions:
                ...
                stream.write(
                    f"""\n- id: '{current_id}'
  prev_id: '{interaction.prev_stateful_case.case_id if interaction.prev_stateful_case is not None else '' }'
  status: '{status}'
  seed: '{item.seed}'
        ...
        while True:
            ...
            if isinstance(item, Process):
                for interaction in item.interactions:
                    ...
                    content_type = interaction.response.headers.get(
                        "Content-Type", [""]
                    )[0]
                    content = harfile.Content(
                        ...
                        text=(
                            get_body(interaction.response.body)
                        ...
                        request=harfile.Request(
                            ...
                            queryString=[
                                ...
                                for name, value in parse_qsl(
                                    query_params, keep_blank_values=True
                                )
                            ],
                            cookies=_extract_cookies(
                                interaction.request.headers.get("Cookie", [])
                            ),
...
def _extract_cookies(headers: list[str]) -> list[harfile.Cookie]:
    return [
        cookie for items in headers for item in items for cookie in _cookie_to_har(item)
    ]
