...
class BaseOpenAPISchema(BaseSchema):
    ...
    def _on_missing_operation(
        self, item: str, exc: KeyError, matches: list[str]
    ) -> NoReturn:
    ...
    def _resolve_shared_parameters(
        self, path_item: Mapping[str, Any]
    ) -> list[dict[str, Any]]:
    ...
    def _collect_operation_parameters(
        ...
        return self.collect_parameters(
            itertools.chain(parameters, shared_parameters), operation
        )
                    ...
                    for method, entry in path_item.items():
                        ...
                        try:
                            ...
                            parameters = collect_parameters(
                                itertools.chain(parameters, shared_parameters), resolved
                            )
                            operation = make_operation(
                                path, method, parameters, entry, resolved, scope
                            )
    ...
    def _into_err(
        self, error: Exception, path: str | None, method: str | None
    ) -> Err[OperationSchemaError]:
        __tracebackhide__ = True
        try:
    ...
    def _raise_invalid_schema(
        ...
        raise OperationSchemaError(
            SCHEMA_ERROR_MESSAGE, path=path, method=method, full_path=full_path
        ) from error
    ...
    def make_operation(
        ...
        self.dispatch_hook(
            "before_init_operation", HookContext(operation=operation), operation
        )
        return operation
    ...
    def get_operation_by_id(self, operation_id: str) -> APIOperation:
        ...
        initialized = self.make_operation(
            entry.path, entry.method, parameters, entry.operation, resolved, entry.scope
        )
        cache.insert_operation(
            initialized, traversal_key=traversal_key, operation_id=operation_id
        )
        return initialized
    ...
    def get_operation_by_reference(self, reference: str) -> APIOperation:
        ...
        initialized = self.make_operation(
            path, method, parameters, operation, resolved, scope
        )
        cache.insert_operation(
            initialized, traversal_key=traversal_key, reference=reference
        )
        return initialized
    ...
    def get_parameter_serializer(
        ...
        definitions = [
            item
            for item in operation.definition.resolved.get("parameters", [])
            if item["in"] == location
        ]
        ...
        security_parameters = [
            item for item in security_parameters if item["in"] == location
        ]
    ...
    def _get_response_definitions(
        ...
        if status_code in responses:
            _, response = self.resolver.resolve_in_scope(
                responses[status_code], operation.definition.scope
            )
            return response
        if "default" in responses:
            _, response = self.resolver.resolve_in_scope(
                responses["default"], operation.definition.scope
            )
            return response
        return None
    ...
    def add_link(
        ...
        possibilities = [
            f"{op.ok().method.upper()} {op.ok().path}"
        ...
        if matches:
            message += f" Did you mean `{matches[0]}`?"
        message += (
            " Check if the requested API operation passes the filters in the schema."
        )
    ...
    def validate_response(
        ...
        responses = {
            str(key): value
            for key, value in operation.definition.raw.get("responses", {}).items()
        }
        status_code = str(response.status_code)
        if status_code in responses:
            ...
            return None
        scopes, schema = self.get_response_schema(
            definition, operation.definition.scope
        )
        if not schema:
            ...
            try:
                raise get_missing_content_type_error(operation.verbose_name)(
                    ...
                    context=failures.MissingContentType(
                        message=message, media_types=media_types
                    ),
                )
    ...
    def rewritten_components(self) -> dict[str, Any]:
        if not hasattr(self, "_rewritten_components"):
            ...
            def callback(_schema: dict[str, Any], nullable_name: str) -> dict[str, Any]:
                _schema = to_json_schema(
                    _schema, nullable_name=nullable_name, copy=False
                )
            ...
            for path in self.component_locations:
                ...
                for chunk in path:
                    ...
                    target.update(
                        traverse_schema(
                            fast_deepcopy(schema), callback, self.nullable_name
                        )
                    )
    ...
    def _rewrite_references(
        self, schema: dict[str, Any], resolver: InliningResolver
    ) -> dict[str, Any]:
...
@contextmanager
def in_scope(
    resolver: jsonschema.RefResolver, scope: str
...
@contextmanager
def in_scopes(
    resolver: jsonschema.RefResolver, scopes: list[str]
) -> Generator[None, None, None]:
...
class MethodMap(Mapping):
    ...
    def _init_operation(self, method: str) -> APIOperation:
        ...
        initialized = schema.make_operation(
            path, method, parameters, operation, resolved, scope
        )
        cache.insert_operation(
            initialized,
...
class SwaggerV20(BaseOpenAPISchema):
    ...
    def collect_parameters(
        ...
        for parameter in parameters:
            if parameter["in"] == "formData":
                ...
                for media_type in body_media_types:
                    collected.append(
                        OpenAPI20Body(definition=parameter, media_type=media_type)
                    )
            else:
        ...
        logger.debug(
            "deps/schemathesis/src/schemathesis/specs/openapi/schemas.py -> collect_param_v2"
        )
    ...
    def get_strategies_from_examples(
        ...
        """Get examples from the API operation."""
        return get_strategies_from_examples(
            operation, as_strategy_kwargs=as_strategy_kwargs
        )
    ...
    def get_response_schema(
        ...
        return scopes, to_json_schema_recursive(
            schema, self.nullable_name, is_response_schema=True
        )
            ...
            if isinstance(parameter, OpenAPI20CompositeBody):
                for form_parameter in parameter.definition:
                    ...
                    if name in form_data:
                        ...
                        if (
                            form_parameter.definition.get("type") == "file"
                            or is_multipart
                        ):
                            add_file(value)
                        else:
    ...
    def _get_payload_schema(
        self, definition: dict[str, Any], media_type: str
    ) -> dict[str, Any] | None:
...
class OpenApi30(SwaggerV20):
    ...
    def _get_base_path(self) -> str:
        ...
        if servers:
            ...
            url = server["url"].format(
                **{k: v["default"] for k, v in server.get("variables", {}).items()}
            )
    ...
    def collect_parameters(
        ...
        logger.debug(
            "deps/schemathesis/src/schemathesis/specs/openapi/schemas.py:collect_params -> class OpenApi30"
        )
        ...
        if "requestBody" in definition:
            ...
            logger.debug(
                ">>>>>>>>>>>>>>>>>>>>>>>> Content in request body: %s", content
            )
        return collected
    ...
    def get_response_schema(
        ...
        if option and "schema" in option:
            ...
            # because it is not converted
            return scopes, to_json_schema_recursive(
                option["schema"], self.nullable_name, is_response_schema=True
            )
        return scopes, None
    ...
    def get_request_payload_content_types(self, operation: APIOperation) -> list[str]:
        request_body = self._resolve_until_no_references(
            operation.definition.raw["requestBody"]
        )
    ...
    def prepare_multipart(
        ...
        files = []
        logger.debug(
            "deps/schemathesis/src/schemathesis/specs/openapi/schemas.py -> prepare_multipart"
        )
        ...
        for media_type, entry in content.items():
            ...
            raise InternalError(
                "No 'multipart/form-data' media type found in the schema"
            )
    ...
    def _get_payload_schema(
        ...
        if "requestBody" in definition:
            if "$ref" in definition["requestBody"]:
                body = self.resolver.resolve_all(
                    definition["requestBody"], RECURSION_DEPTH_LIMIT
                )
            else:
